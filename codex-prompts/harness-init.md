---
description: 给项目初始化一套 AI 可长期开发的工程环境（agent harness）：探测技术栈 → AGENTS.md 地图 + docs/ + exec-plans + scripts 统一验证入口
---
...
三件套之一：`/harness-init`（建轨道，本命令）→ `/harness-task`（每轮任务沿轨道推进）→ `/harness-maintain`（定期养轨道）。
...
- **技术栈**：package.json / pyproject.toml / Cargo.toml / go.mod / CMakeLists.txt / Makefile → 确定 dev / typecheck / lint / test 的**真实**命令。
...
模板与 Claude Code 共用同一份，在 `~/.claude/skills/harness-init/assets/`（Windows：`%USERPROFILE%\.claude\skills\harness-init\assets\`）。读取模板，按第一步探测到的事实填充；模板目录不存在时按同样结构自行生成骨架。

| 目标文件 | 模板 |
|---|---|
| `AGENTS.md` | `AGENTS.template.md` |
| `docs/PRODUCT.md` | `PRODUCT.template.md` |
| `docs/ARCHITECTURE.md` | `ARCHITECTURE.template.md` |
| `docs/TESTING.md` | `TESTING.template.md` |
| `docs/exec-plans/template.md` | `exec-plan.template.md` |

关键动作：

1. `AGENTS.md` 填入真实命令与真实文档链接（没建的文档不要列）。
2. **CLAUDE.md 衔接**：项目没有 CLAUDE.md → 创建一个只含 `@AGENTS.md` 一行的 CLAUDE.md（Claude Code 会自动导入，Codex 直接读 AGENTS.md，一份地图两边共用）；已有 CLAUDE.md → 在其开头加一行指向 AGENTS.md，不动既有内容。
3. `docs/exec-plans/active/` 与 `completed/` 建空目录（放 `.gitkeep`）。

## 第四步：scripts 统一入口

把项目**已有**的真实命令包装为统一入口（bash + `set -euo pipefail`，`chmod +x`；纯 Windows 项目可生成 PowerShell 等价脚本，但入口名与职责保持一致）：
